...
function install() {
  ...
  try {
    ...
    console.error(`[teamWERX][ci-prompt] TEAMWERX_CI set but inquirer not available: ${err.message}`);
    return null;
  }

  if (!inquirer || typeof inquirer.prompt !== 'function') {
    // eslint-disable-next-line no-console
    console.error('[teamWERX][ci-prompt] inquirer.prompt is not a function - skipping CI shim');
    return null;
  }
  ...
  inquirer.prompt = async (questions) => {
    try {
      ...
      // Log a brief diagnostic so CI logs show what we auto-answered.
      // eslint-disable-next-line no-console
      console.error(`[teamWERX][ci-prompt] auto-answers: ${JSON.stringify(answers)}`);
      ...
      // eslint-disable-next-line no-console
      console.error(`[teamWERX][ci-prompt] error auto-answering prompt: ${err && err.message ? err.message : String(err)}; falling back to interactive prompt`);
