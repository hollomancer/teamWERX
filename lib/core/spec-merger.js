...
class SpecMerger {
  ...
  async _mergeDomain(domain, deltaPath, baseFingerprint, opts = {}) {
    ...
    if (baseFingerprint) {
      ...
      if (diverged) {
        ...
        if (opts.force) {
          // proceed with merge despite divergence
        } else {
          const autoMergeResult = await this._autoMerge(
            domain,
            delta,
            baseFingerprint,
            currentSpec
          ...
          if (autoMergeResult.success) {
            // Auto-merge was successful, so we can proceed
            // The merged content is returned from autoMerge
            const mergedContent = autoMergeResult.mergedContent;
            if (!opts.dryRun) {
              await this.specManager.writeSpec(domain, mergedContent);
            }
            return { ...result, ...autoMergeResult.mergeReport };
          } else {
            result.conflicts.push(...autoMergeResult.conflicts);
            throw new Error(
              `Spec divergence detected for domain '${domain}'. Auto-merge failed.`
            );
          }
        }
      }
  ...
  async _autoMerge(domain, delta, baseFingerprint, currentSpec) {
    const divergenceDetails = this._analyzeDivergence(
      baseFingerprint.requirements || [],
      currentSpec.requirements
    );

    const conflictedRequirementIds = new Set(
      divergenceDetails.map((d) => d.requirement)
    );

    const conflicts = [];
    const cleanOperations = { ADDED: [], MODIFIED: [], REMOVED: [] };

    for (const op of this.validOperations) {
      for (const req of delta.operations[op]) {
        if (conflictedRequirementIds.has(req.id)) {
          const baseReq = (baseFingerprint.requirements || []).find(
            (r) => r.id === req.id
          );
          const currentReq = currentSpec.requirements.find(
            (r) => r.id === req.id
          );

          conflicts.push({
            type: "AUTO_MERGE_CONFLICT",
            domain,
            requirement_id: req.id,
            message: `Requirement '${req.id}' has been modified in both the current spec and the delta.`,
            base: baseReq ? baseReq.content : null,
            current: currentReq ? currentReq.content : null,
            delta: req.content,
          });
        } else {
          cleanOperations[op].push(req);
        }
      }
    }
    ...
    // If no conflicts, then we can auto-merge
    const mergeReport = {
      operations_applied: {
        ADDED: 0,
        MODIFIED: 0,
    ...
    const mergedContent = this._applyOperations(
      ...
      currentSpec.requirements,
      cleanOperations,
      { operations_applied: mergeReport.operations_applied, conflicts: [] } // Pass a clean result object
    );

    return { success: true, mergedContent, mergeReport };
  }
}
