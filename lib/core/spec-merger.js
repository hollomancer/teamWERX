    ...
    if (baseFingerprint) {
      ...
      if (diverged) {
        ...
        result.conflicts.push({
          type: 'DIVERGENCE',
          domain,
          message: `Spec has changed since change was created (base: ${baseFingerprint.base_fingerprint}, current: ${currentSpec.fingerprint})`,
        });

        if (!opts.force) {
          throw new Error(
            `Spec divergence detected for domain '${domain}'. Use --force to override or run 'teamwerx changes sync' to update fingerprints.`
          );
        }
      }
    }
