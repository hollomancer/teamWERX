...
class SpecDeltaParser {
  ...
  async writeDeltaFile(deltaPath, delta) {
    const metadata = {
      change: delta.change,
      domain: delta.domain,
      delta_type: "spec",
    ...
    let content = `# Spec Delta: ${delta.domain}\n`;

    for (const op of this.validOperations) {
      if (delta.operations[op] && delta.operations[op].length > 0) {
        content += `\n## ${op} Requirements\n\n`;
    ...
    await writeFileWithFrontmatter(deltaPath, metadata, content);
  }
  ...
  parseDelta(content, metadata = {}) {
    ...
    for (const section of sections) {
      if (
        section.operation &&
        this.validOperations.includes(section.operation)
      ) {
        const requirements = this._parseRequirementsFromSection(
          section.content
        );
  ...
  _splitIntoSections(content) {
    ...
    for (const line of lines) {
      ...
      const sectionMatch = line.match(
        /^## (ADDED|MODIFIED|REMOVED) Requirements$/i
      );
      ...
      if (
        currentSection &&
        line.match(/^## /) &&
        !line.match(/^## (ADDED|MODIFIED|REMOVED)/i)
      ) {
