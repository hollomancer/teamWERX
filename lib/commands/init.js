...
async function init() {
  ...
  if (await dirExists(teamwerxPath)) {
    spinner.stop();
    const { reinit } = await prompts.prompt([
      {
        type: "confirm",
        name: "reinit",
        message:
          "teamWERX is already initialized. Do you want to reinitialize?",
        default: false,
      },
    ]);
  ...
  console.log(
    chalk.gray("  1. Create your first goal: ") +
      chalk.cyan('teamwerx goal "Your goal description"')
  );
  console.log(
    chalk.gray("  2. List all goals: ") + chalk.cyan("teamwerx list")
  );
  console.log(chalk.gray("  3. View help: ") + chalk.cyan("teamwerx --help"));
  console.log(
    chalk.gray("\nRemember to commit your .teamwerx files with: ") +
      chalk.cyan("[teamWERX] prefix\n")
  );
}
