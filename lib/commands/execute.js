...
async function execute(goalName) {
  ...
  if (pendingTasks.length === 0) {
    ...
    console.log(chalk.gray('Consider archiving this goal: ') + chalk.cyan(`teamwerx archive ${goal}\n`));
    return;
  }
  ...
  console.log(chalk.white('Task: ') + chalk.cyan(`${nextTask.id} - ${nextTask.title}`));
  ...
  console.log(chalk.white('Remaining: ') + chalk.gray(`${pendingTasks.length - 1} pending tasks\n`));
  
  console.log(chalk.gray('When complete, run: ') + chalk.cyan(`teamwerx complete --source batch\n`));
}
