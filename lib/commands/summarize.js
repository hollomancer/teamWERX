...
const { getCurrentGoal, fileExists, readFileWithFrontmatter, writeFileWithFrontmatter } = require('../utils/file');
...
async function summarize(options = {}) {
  ...
  if (exists) {
    ...
    console.log(chalk.gray('Update the existing summary at: ') + chalk.cyan(summaryPath));
...
`;
  ...
  console.log(chalk.gray('Review discussions, research, and implementations to complete the summary.\n'));
}
