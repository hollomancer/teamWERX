...
async function ensureUniqueTarget(targetPath) {
  ...
  // Fallback: should never happen, but throw to avoid an infinite loop
  throw new Error(`Unable to find unique filename for ${targetPath}`);
}
...
async function archive(goalName, options = {}) {
  ...
  if (!options.yes) {
    ...
    const { confirmArchive } = await prompts.prompt([
      {
        type: "confirm",
        name: "confirmArchive",
  ...
  for (const item of itemsToArchive) {
    try {
      ...
      console.error(
        chalk.red(
          `✗ Failed to archive ${item.label.toLowerCase()}: ${err.message}`
        )
      );
    }
  }
