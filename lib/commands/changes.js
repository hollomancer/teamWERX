 ...
 *
 * This CLI module provides thin, user-facing commands that orchestrate the
...
async function propose(title, options = {}) {
  try {
    ...
    console.log(chalk.white("  • Review: teamwerx changes show " + created.id));
    console.log("");
  } catch (err) {
    console.error(chalk.red("Error creating proposal:"), err.message || err);
    process.exit(1);
  }
}

async function listChanges(_unused, options = {}) {
  try {
    const changeManager = new ChangeManager();
    ...
    for (const ch of changes) {
      ...
      console.log(
        `${chalk.cyan(ch.id)} ${chalk.white("-")} ${chalk.bold(
          ch.title || ""
    ...
    console.error(chalk.red("Error listing changes:"), err.message || err);
    process.exit(1);
  }
}

async function show(changeId, options = {}) {
  try {
      ...
      if (change.tasks.parsed && change.tasks.parsed.length) {
        for (const t of change.tasks.parsed) {
          console.log(
            "  - " +
              chalk.white(t.title) +
              (t.idHint ? chalk.gray(" (hint: " + t.idHint + ")") : "")
          );
        }
    ...
    console.error(chalk.red("Error showing change:"), err.message || err);
    process.exit(1);
  }
}

async function apply(changeId, options = {}) {
  try {
    if (!changeId || !String(changeId).trim()) {
    ...
    const createImplementations = options.createImplementations !== false; // default true
      ...
      const { confirmApply } = await inquirer.prompt([
        {
          type: "confirm",
          name: "confirmApply",
          message: `Apply change ${changeId} to goal ${
            goal || "<unspecified>"
          }?`,
          default: false,
        },
    ...
    console.error(chalk.red("Error applying change:"), err.message || err);
    process.exit(1);
  }
}

async function archive(changeId, options = {}) {
  try {
    if (!changeId || !String(changeId).trim()) {
      console.error(
      ...
      const { confirm } = await inquirer.prompt([
        {
          type: "confirm",
          name: "confirm",
          message: `Archive change ${changeId}? This will move it to .teamwerx/archive/changes/`,
          default: false,
        },
      ]);
      if (!confirm) {
    ...
    console.error(chalk.red("Error archiving change:"), err.message || err);
    process.exit(1);
  }
}
