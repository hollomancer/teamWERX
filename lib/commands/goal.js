...
async function goal(description) {
  ...
  if (!goalTitle) {
    console.log(chalk.gray('💡 Tip: Use a clear, actionable goal statement (e.g., "Implement user authentication system")'));
    console.log();
    ...
    const answers = await inquirer.prompt([
      {
        ...
        validate: (input) => {
          ...
          if (trimmed.length < 5) return 'Goal description should be at least 5 characters';
          if (trimmed.length > 100) return 'Goal description should be less than 100 characters';
          return true;
        }
      }
    ]);
  ...
  console.log(chalk.gray('Examples: "P1: Users can log in with email/password", "P2: Password reset functionality"'));
  console.log();
  ...
  while (true) {
    const { criterion } = await inquirer.prompt([
      {
        ...
        validate: (input) => {
          const trimmed = input.trim();
          if (trimmed.length > 200) return 'Criterion should be less than 200 characters';
          return true;
        }
      }
    ]);
    ...
    if (!criterion.trim()) {
      if (successCriteria.length === 0) {
        console.log(chalk.yellow('⚠ At least one success criterion is required.'));
        continue;
      }
      break;
    }
    ...
    // Show progress
    console.log(chalk.green(`✓ Added: ${successCriteria[successCriteria.length - 1]}`));
  }
  ...
  console.log(chalk.gray('Define how you\'ll measure success. This helps track progress quantitatively.'));
  console.log(chalk.gray('Examples: "User registration conversion rate", "API response time", "Test coverage percentage"'));
  console.log();
    ...
    const outcomeAnswers = await inquirer.prompt([
      {
        ...
        validate: (input) => {
          ...
          if (trimmed.length > 50) return 'Metric name should be less than 50 characters';
          return true;
        }
      },
      {
        ...
        validate: (input) => {
          ...
          if (trimmed.length > 30) return 'Target value should be less than 30 characters';
          return true;
        }
      },
      {
        type: 'input',
        name: 'measurement',
        message: '📐 How to measure (e.g., "Post-implementation survey", "Performance monitoring"):',
        validate: (input) => {
          const trimmed = input.trim();
          if (trimmed.length === 0) return 'Measurement method is required';
          if (trimmed.length > 100) return 'Measurement description should be less than 100 characters';
          return true;
        }
      }
    ]);
