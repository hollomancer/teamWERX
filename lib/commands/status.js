...
async function showGoalStatus(goalName, options = {}) {
  ...
  if (await fileExists(planPath)) {
    ...
    const completed = plan.data.tasks ? plan.data.tasks.filter(t => t.status === 'completed').length : 0;
    const total = plan.data.tasks ? plan.data.tasks.length : 0;
    planData = {
      exists: true,
  ...
  if (planData) {
    ...
    console.log(FORMAT.labelValue('Tasks:', `${planData.tasks.completed}/${planData.tasks.total} completed`));
  }
...
async function showAllGoalsStatus(options = {}) {
  ...
  const result = {
    currentGoal,
    goals: goalsWithStatus.map(goal => ({
      name: goal.filename,
      title: goal.title,
      status: goal.status,
      created: goal.created,
      isCurrent: goal.filename === currentGoal
    })),
    summary: {
      total: goalsWithStatus.length,
      byStatus: Object.keys(grouped).reduce((acc, status) => {
  ...
  for (const status of statuses) {
    if (grouped[status] && grouped[status].length > 0) {
      console.log(FORMAT.status(status.toUpperCase()) + ` (${grouped[status].length}):`);
      grouped[status].forEach(goal => {
        const marker = currentGoal === goal.filename ? '●' : '○';
        console.log(FORMAT.listItem(`${goal.title} (${goal.filename})`, marker));
      });
      console.log();
    }
  }
