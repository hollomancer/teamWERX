...
describe("ChangeManager (core)", () => {
  ...
  test("createChange scaffolds proposal, tasks, spec, and status", async () => {
    ...
    // Ensure files exist
    const statProposal = await fs
      .stat(proposalPath)
      .then(() => true)
      .catch(() => false);
    const statTasks = await fs
      .stat(tasksPath)
      .then(() => true)
      .catch(() => false);
    const statSpec = await fs
      .stat(specPath)
      .then(() => true)
      .catch(() => false);
    const statStatus = await fs
      .stat(statusPath)
      .then(() => true)
      .catch(() => false);
...
`;
    ...
    const result = await cm.applyChange(res.id, {
      goal: "alpha-goal",
      createImplementations: true,
    });
